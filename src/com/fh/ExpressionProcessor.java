...
public class ExpressionProcessor {
	...
	public static final int NULL_EXPRESSION = 0;	//杈撳叆琛ㄨ揪寮忎负绌�
	public static final int GRAMMAR_ERROR = 1;	//琛ㄨ揪寮忚娉曢敊璇�
	public static final int NONE_VARIABLE = 2;	//琛ㄨ揪寮忓彉閲忎笉瀛樺湪
	public static final int NULL_INPUT = 3;	//杈撳叆涓虹┖
	public static final String[] ERROR_MESSAGES = {"Error, please input an inital expression!", "Error, input error!", "Error, no such variable!", "Error, please input!"};
	...
	public void expression (String tempInput) {
		...
		if (tempInput.charAt(postIndexOfExpression) != '!') {	//濡傛灉鏄師濮嬭〃杈惧紡
			...
			while (postIndexOfExpression < input.length()) {
				...
				else if (Character.isDigit(input.charAt(postIndexOfExpression))) {	//褰撳墠澶勭悊瀛楃涓烘暟瀛�
					String temp = "";
				...
				else if (input.charAt(postIndexOfExpression) == '+' || input.charAt(postIndexOfExpression) == '*') {	//褰撳墠澶勭悊瀛楃涓哄垎闅旂
					if (postIndexOfExpression == input.length() - 1) {
						this.isLegal = false;
						this.proceException(GRAMMAR_ERROR);
						return ;
			...
			for (postIndexOfExpression = 0; postIndexOfExpression <= expression.length(); postIndexOfExpression++) {
				...
				Element subTempExpression = new Element();	//鍘熷琛ㄨ揪寮忓瓙椤�
				String doubleSubTemp;	//瀛愰」涓殑鍥犲紡
				int i, j;
				i = j = 0;
			...
			if (tempInput.length() == 5 && tempInput.substring(1, 4).equals("d/d")) {
				...
				while (postIndexOfExpression < tempInput.length()) {
					if (Character.isLetter(tempInput.charAt(postIndexOfExpression))) {
					...
					else if (Character.isDigit(tempInput.charAt(postIndexOfExpression))) {
						String temp = "";
	...
	public void derivation (String input) {
		...
		for (int i = 0; i < derivateExpression.size(); i++) {	//閬嶅巻琛ㄨ揪寮忓瓙椤�
			if (derivateExpression.get(i).getIsVariable()) {	//瀛愰」瀛樺湪鍙橀噺
				int count = 0;
				int j = 0;
...
class Element {
	private boolean isVariable = false;	//瀛愰」鏄惁瀛樺湪鍙橀噺
	private double coefficient = 1;	//瀛愰」绯绘暟
	private String variables = "";	//瀛愰」鍙橀噺
